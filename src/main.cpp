...
void show_debug_panel(InputSystem &input_system, Features &features, float &cameraScalar) {
    auto windowFlags = ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoNavFocus |
...
void show_control_panel(ChargeSystem &charge_system, Cursor_Point &cursor) {
    ...
    for (auto &charge: charge_system.getChargesList()) {
        ...
        charge.color = Color{
            (unsigned char) (temp_color[0] >= 1.0
                                 ? 255
                                 : (temp_color[0] <= 0.0 ? 0 : (int) floor(temp_color[0] * 256.0))),
            (unsigned char) (temp_color[1] >= 1.0
                                 ? 255
                                 : (temp_color[1] <= 0.0 ? 0 : (int) floor(temp_color[1] * 256.0))),
            (unsigned char) (temp_color[2] >= 1.0
                                 ? 255
                                 : (temp_color[2] <= 0.0 ? 0 : (int) floor(temp_color[2] * 256.0))),
            255
        };
    ...
    if (features.showResultPanel) {
        if (charge_system.isChargeSystemValid()) {
            ...
            for (const auto &charge_result: charge_system.getMainCursor().cursor_result) {
                auto particle_color =
                        ImColor(charge_result.particle_color.r, charge_result.particle_color.g,
                                charge_result.particle_color.b);
                ImGui::TextColored(particle_color, "PARTICLE NO %d", index);
...
void load_fonts(Fonts &fonts, float base_scale) {
    ...
    if (base_scale != previous_scale) {
        ...
        //TODO make it agnostic from the platform
        //Default font
        ...
        fonts.font_normal_panel = io.Fonts->AddFontFromFileTTF(R"(/usr/share/fonts/gnu-free/FreeMono.otf)",
                                                               base_scale * 15);
        fonts.font_drag_panel = io.Fonts->AddFontFromFileTTF(R"(/usr/share/fonts/gnu-free/FreeMono.otf)",
                                                             base_scale * 20);
        fonts.font_error_panel = io.Fonts->AddFontFromFileTTF(R"(/usr/share/fonts/gnu-free/FreeMono.otf)",
                                                              base_scale * 35);
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (first_time) {
            ...
            auto dock_id_right = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Right, 0.20f, nullptr,
                                                             &dockspace_id);
            ...
            if (charge_system.isChargeSystemValid() && input_system.keysStatus.verbose) {
                for (const auto &charge: charge_system.getChargesList()) {
                    ...
                    ImVec2 windowPos = ImVec2{
                        origin.x + GLOBAL_SCALE * camera.zoom * charge.position.x,
        ...
        if (!features.showHelpPanel && !features.showMoreHelpPanel) {
            ...
            static std::vector<Color>
                    charge_color_list = {
                        Color(RED), Color(BLUE), Color(ORANGE), Color(GRAY), Color(GREEN), Color(MAROON)
                    };
            ...
            if (!ImGui::GetIO().WantCaptureMouse) {
                if (IsKeyPressed(KEY_Q) && !IsKeyDown(KEY_LEFT_SHIFT)) {
                    charge_system.addCharge(input_system.getRefMousePos(), 10, NANO, 0.1,
                                            charge_color_list[charges_color_index]);
                    charges_color_index++;
                }
            }
            if (input_system.keysStatus.debug) {
                ...
                if (!cursor_dragged) {
                    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
                        if (charge_last_pressed_element != -1) {
                            charge_system.getChargesList()[charge_last_pressed_element].position = input_system.
                                    getRefMousePos();
                            ...
                            for (auto &charge: charge_system.getChargesList()) {
                                if (CheckCollisionPointCircle(input_system.getRefMousePos(),
                                    ...
                                    show_mouse_position_panel(fonts.font_drag_panel, input_system, currentCameraZoom,
                                                              camera);
                ...
                if (!charge_dragged) {
                    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
                        if (cursor_last_pressed_element != -1) {
                            charge_system.getCursorList()[cursor_last_pressed_element].position = input_system.
                                    getRefMousePos();
                            show_mouse_position_panel(fonts.font_drag_panel, input_system, currentCameraZoom, camera);
                        } else {
                            for (auto &cursor: charge_system.getCursorList()) {
